...
export function QuestCheckpointRestoreDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[min(88vh,640px)] max-w-md gap-3 border-[var(--candle-rule)] bg-[var(--candle-panel)] p-4 font-serif text-[var(--candle-wax)]">
        <DialogHeader>
          <DialogTitle className="font-serif text-lg text-[var(--candle-wax)]">
            Restore checkpoint
          </DialogTitle>
          <DialogDescription className="text-[0.75rem] leading-snug text-[var(--candle-ink-soft)]">
            Kind 10032 history for your npub (newest first). Loading one re-publishes it so it becomes
            the active save on next login.
          </DialogDescription>
        </DialogHeader>
        ...
        {!myPubkey ? (
          ...
          <ScrollArea className="max-h-[min(52vh,420px)] rounded-md border border-[var(--candle-rule)]/60">
                ...
                return (
                  <li key={row.eventId} className="flex items-start gap-2 px-3 py-2.5">
                    ...
                    <Button
                      ...
                      size="sm"
                      className={cn(
                        'shrink-0 font-serif text-[0.65rem] uppercase tracking-wide',
                        'border-[var(--candle-rule)]/70 bg-black/25 hover:bg-black/40'
                      )}
        ...
        {myPubkey && rows.length > 0 ? (
          <p className="text-[0.65rem] leading-snug text-[var(--candle-ink-faint)]">
            Showing up to {rows.length} event{rows.length === 1 ? '' : 's'}. Older rows may exist if
            relays trimmed history.
          </p>
        ) : null}
      </DialogContent>
    </Dialog>
